...
function FirstDay(props) {
  ...
  //   fetchData();
  // }, []);
  const isHolyday = props.colombianHolydays.some(
    (festivo) =>
      festivo.dia === props.dia && festivo.mes === props.monthCalendario
  );
  const dayStart =
    new Date(props.yearSet, props.monthCalendario, 1).getUTCDay() + 1; // retorna el primer dia de la semana sumando 1 unidad
